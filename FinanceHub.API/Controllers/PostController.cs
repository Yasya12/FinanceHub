    ...
    : BaseController(mediator, logger)
{
    ...
    [HttpGet("hub-posts")]
    public async Task<ActionResult<PagedList<GetPostDto>>> GetAllHubPost([FromQuery] PostParams postParams,
    ...
    public async Task<ActionResult<PagedList<GetPostDto>>> GetHubPostsWithLikes([FromQuery] PostParams postParams,
        [FromQuery] Guid hubId)
    {
    ...
    public async Task<ActionResult<PagedList<GetPostDto>>> GetAllFollowingPostWithLikes(
    ...
    public async Task<ActionResult<PagedList<GetPostDto>>> GetPostsForSpecificUserWithLikes(
        [FromQuery] PostParams postParams, [FromQuery] Guid specificUserId)
    {
        var currentUserId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (currentUserId == null) return BadRequest("No User Id found in token");
        ...
        var paginatedPosts =
            await postService.GetPostsForSpecificUserWithLikesAsync(postParams, userId, specificUserId);
    ...
    public async Task<ActionResult<PagedList<GetPostDto>>> GetLikedPostsForSpecificUserWithLikes(
        [FromQuery] PostParams postParams, [FromQuery] Guid specificUserId)
    {
        var currentUserId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (currentUserId == null) return BadRequest("No User Id found in token");
        ...
        var paginatedPosts =
            await postService.GetLikedPostsForSpecificUserWithLikesAsync(postParams, userId, specificUserId);
